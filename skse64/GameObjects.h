...
public:
	...
	DEFINE_MEMBER_FN(GetCostliestEffectItem, EffectItem *, 0x00151A00, int arg1, bool arg2);
	DEFINE_MEMBER_FN(GetEffectiveMagickaCost, float, 0x00151780, Character* caster);
};
...
public:
	...
	DEFINE_MEMBER_FN(GetSex, char, 0x003AFEB0);	// return (formType == 0x2B) ? (actorData.flags & 1) : -1;
	DEFINE_MEMBER_FN(HasOverlays, bool, 0x003C8C00);

	DEFINE_MEMBER_FN(ChangeHeadPart, void, 0x003C5BD0, BGSHeadPart * target);	// Swaps a headPart of the same type as target with target
	...
	DEFINE_MEMBER_FN(UpdateNeck, void, 0x003C0080, BSFaceGenNiNode * faceNode);	// Updates the neck seam when weight changed

	DEFINE_MEMBER_FN(SetSkinFromTint, void, 0x003BFE40, NiColorA * result, TintMask * tintMask, bool fromTint);	// Computes RGB SkinTone from RGBA TintMask
